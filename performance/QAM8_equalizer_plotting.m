function QAM8_equalizer_plotting()

%% System Parameters

modulation_type = 'QAM';           % Modulation type ('PAM' for BPSK, 'QAM' for 8-QAM or 16-QAM)
constellation_size = 8;            % Constellation size (2, 8, or 16)
...
eb_n0_db = 0:1:30;                             % Eb/N0 range in dB
...
for channel_id = 1:3
    ...
    % Generate ALL the noise at once with element-wise vector multiplication (.*)
    ...
    % Reference matrix containing duplicated bits in columns (to compute BER at the end)
    ...
    % Demodulation: convert the selected symbols to bits
    ...
    % Feedback loop, from the end to the beginning
    for k = frame_length:-1:1
        ...
        % Subtract the interference and normalize
        soft_symbols = (z_tilde_all(k, :) - interference) / R(k, k);
        
        % Compute the distances for the decision
    ...
    % Demodulation: convert the chosen symbols to bits
